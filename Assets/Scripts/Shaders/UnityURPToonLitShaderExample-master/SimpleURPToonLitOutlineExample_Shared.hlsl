...
CBUFFER_START(UnityPerMaterial)
    ...
    //Specular Light
    float   _UseSpecularLight;
    half    _SpecularLightAmount;
...
half3 ShadeAllLights(ToonSurfaceData surfaceData, ToonLightingData lightingData)
{
    ...
    // High light
    half3 highLightResult = {0,0,0};
    if(_UseSpecularLight){
        highLightResult = ShadeHighLight(surfaceData, lightingData, mainLight);
    }
    ...
    return CompositeAllLightResults(indirectResult, mainLightResult, additionalLightSumResult, emissionResult, highLightResult, surfaceData, lightingData);
}
...
half4 ShadeFinalColor(Varyings input) : SV_TARGET
{
    ...
    float distanceFromCamera = distance(input.positionCS, _WorldSpaceCameraPos);
    half fade = saturate((distanceFromCamera - _FadeMinDistance) / _FadeMaxDistance);

    return half4(color, surfaceData.alpha * 0.3);
}
